...
class YiWengeTranslator(BaseTranslator):
    name = "yi_wenge"
    # envs = {
    #     "YI_BASE_URL": "http://yi.wenge.com:20026/fanyi/openapi/v1/text/translate",
    #     "YI_API_KEY": "2582b831afc84db8b20b6b44d47a0c3a",
    # }

    envs = {
        "YI_BASE_URL": config['YI_BASE_URL'],
        "YI_API_KEY": config['YI_API_KEY'],
    }

    def __init__(self, lang_in, lang_out, model="default", envs=None, ignore_cache=True, **kwargs):
        # 1. 设置环境变量
        self.set_envs(envs)
        
        # 2. 检查必要参数
        # if not self.envs["YI_API_KEY"]:
        #      raise ValueError("The YI_API_KEY is missing.")
        # print("kwargs 中的所有内容：", kwargs)
        self.api_key = self.envs["YI_API_KEY"]
        self.base_url = self.envs["YI_BASE_URL"]
        # self.termbase_id = termbase_id
        # print("Yi Wenge Translator Initialized.")
        # 3. 初始化父类 (设置语言映射、缓存等)
        super().__init__(lang_in, lang_out, model, ignore_cache)

    def do_translate(self, text: str) -> str:
        """
        重写具体的翻译逻辑，使用 requests 调用 Yi Wenge 翻译 API
        """
        headers = {
        ...
        payload = {
            ...
            "sourceLang": self.lang_in
        }

        # if self.termbase_id is not None:
        #     payload["termbaseId"] = self.termbase_id
        # print(self.base_url)
        response = requests.post(self.base_url, headers=headers, json=payload, timeout=200)
        response.raise_for_status()
        
        res_json = response.json()
        
        # 根据实际返回结构：{"code":200, "data": {"context": "你好", ...}}
        return res_json["data"]["context"]
